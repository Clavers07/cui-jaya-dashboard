...
import com.example.konsinyasi.databinding.ActivityPemasukanDetailBinding
...
//        Toast.makeText(this, id_sumber, Toast.LENGTH_LONG).show()
        ...
        when (action) {
            0 -> { // insertion
                ...
                binding.containerNilai.visibility = View.GONE
                ...
                binding.simpan.setOnClickListener {
                    ...
                    val nilai_titipan = Formatter.onlyInt(binding.nilaiTitipan.text.toString())
                    val nilai_komisi = Formatter.onlyInt(binding.nilaiKomisi.text.toString())
                    val catatan = binding.catatan.text.toString()

                    // Cek apakah input valid
                    if (nilai_titipan != null && nilai_komisi != null) {

                        // Menambahkan data ke database
                        val res = db.addPemasukan(id_sumber.toString().toInt(), nilai_titipan, 0, nilai_komisi, catatan)
...
//                    Toast.makeText(this, intent.getIntExtra("ID", 0).toString(), Toast.LENGTH_LONG).show()
                    binding.nama.setText(intent.getStringExtra("NAMA"))
                    binding.alamat.setText(intent.getStringExtra("ALAMAT"))
                    binding.nilaiTitipan.setText(Formatter.easyRead(intent.getIntExtra("TITIPAN", 0)))
                    binding.nilai.setText(Formatter.easyRead(intent.getIntExtra("NILAI", 0)))
                    binding.nilaiKomisi.setText(Formatter.easyRead(intent.getIntExtra("KOMISI", 0)))
...
//                    Toast.makeText(this, binding.id.text.toString().replace("ID: ", ""), Toast.LENGTH_LONG).show()
                ...
                binding.simpan.setOnClickListener {
                    ...
                    val titipan = Formatter.onlyInt(binding.nilaiTitipan.text.toString())
                    val nilai = Formatter.onlyInt(binding.nilai.text.toString())
                    val komisi = Formatter.onlyInt(binding.nilaiKomisi.text.toString())
                    val catatan = binding.catatan.text.toString()

                    // Cek apakah nama kosong
                    if (nilai != null) {
                        // Coba untuk menghapus data berdasarkan nama
                        val updated = db.updatePemasukan(id.toString(), titipan, nilai, komisi, catatan)
