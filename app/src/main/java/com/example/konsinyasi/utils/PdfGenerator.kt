...
class PdfGenerator {
    ...
    fun pemasukanPdf(context: Context, data: List<Pemasukan>, date: String, fileName: String = "database_data", filter: String = "") {
        ...
        try {
            ...
            // Add a title
            val titleFont = Font(Font.FontFamily.HELVETICA, 18f, Font.BOLD, BaseColor.BLACK)
            val title = Paragraph("DATA PENDAPATAN\nKONSINYASI", titleFont)
            title.alignment = Element.ALIGN_CENTER
            document.add(title)
            ...
            val namaHeader = PdfPCell(Phrase("Mitra", headerFont))
            namaHeader.horizontalAlignment = Element.ALIGN_CENTER
            table.addCell(namaHeader)

            val nilaiPemasukanHeader = PdfPCell(Phrase("Nilai Pendapatan", headerFont))
...
//                table.addCell(item.tanggal)      // Tanggal
    ...
    fun bahanPdf(context: Context, data: List<Pengeluaran>, date: String, fileName: String = "database_data", filter: String = "") {
...
//                table.addCell(item.tanggal)      // Tanggal
    ...
    fun sumberPdf(context: Context, data: List<SumberJoinTotal>, date: String, fileName: String = "database_data") {
        ...
        try {
            ...
            // Add a title
            val titleFont = Font(Font.FontFamily.HELVETICA, 18f, Font.BOLD, BaseColor.BLACK)
            val title = Paragraph("DATA MITRA\nKONSINYASI", titleFont)
            title.alignment = Element.ALIGN_CENTER
            document.add(title)
